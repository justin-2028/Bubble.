...
export function XAxis({ category }: Props) {
  const ticks = useMemo(() => {
    ...
    for (let d = 0; d <= limit; d += step) {
      ...
      items.push({ value: d, xPct: mapToViewportPercent(xRight) });
    }
    // Ensure leftmost (limit) and rightmost (0)
    if (!items.find((t) => t.value === limit)) items.push({ value: limit, xPct: mapToViewportPercent(0) });
    if (!items.find((t) => t.value === 0)) items.push({ value: 0, xPct: mapToViewportPercent(100) });
    ...
    <div className="pointer-events-none absolute bottom-0 left-0 right-0 z-10 select-none">
      <div className="relative mb-3 h-12">
        ...
        <div
          ...
          style={{ left: `${VIEWPORT_PAD_LEFT}%`, right: `${VIEWPORT_PAD_RIGHT}%` }}
        />
        {ticks.map((t, idx) => (
          <div
            key={idx}
          ...
          >
            ...
            <div className="absolute left-1/2 top-full -translate-x-1/2 mt-1 text-[11px] font-code text-black/85">
              {t.value}d
            </div>
          </div>
        ))}
